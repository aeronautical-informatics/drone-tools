...
var FirmwareSelector = React.createClass({
    ...
    renderVersionSelect: function renderVersionSelect() {
        ...
        var versions = this.props.selectJESC ? this.props.firmwareVersions['JESC'] : this.props.firmwareVersions['Telemetry'];

        var options = [];
        versions.forEach(function (version, idx) {
            if (version.hasOwnProperty('supportsL') && version.supportsL == false && _this.state.selectedEsc.includes('_L_')) return;
            ...
            React.createElement(
                ...
                React.createElement(
                    ...
                    React.createElement(
                        ...
                        this.props.selectJESC ? 'JESC Version' : 'Telemetry Version'
                    ),
                    options
                ),
                React.createElement(
    ...
    onlineFirmwareSelected: function () {
        var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
            var versions, version, escs, url, cacheKey, hex, eep, _hex;
                ...
                while (1) {
                    switch (_context.prev = _context.next) {
                        ...
                        case 2:
                            ...
                            versions = this.props.selectJESC ? this.props.firmwareVersions['JESC'] : this.props.firmwareVersions['Telemetry'];
                            version = versions[this.state.selectedVersion];
                            escs = this.props.supportedESCs.layouts[this.state.type];

                            if (!this.props.selectJESC) {
                                _context.next = 31;
                                break;
                            }

                            // @todo this replace-based conversion does not work for some ESC files, add a lookup table
                            url = version.url.format(escs[this.state.selectedEsc].name.replace('-H-', '').toUpperCase(), escs[this.state.selectedEsc].name.replace('-H-', 'H').replace('-L-', 'L').toUpperCase(), this.state.selectedMode);
                        ...
                        case 31:
                            _hex = new String(version.url);

                            this.props.onFirmwareLoaded(_hex, undefined);
